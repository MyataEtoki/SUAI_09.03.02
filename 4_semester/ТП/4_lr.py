...
class BinaryTree:
    ...
    def _delete_node(self, node):
        ...
        else: # выбираем преемника - левый или правый ребёнок (минимальный в правом)
            successor = self._minimum(node.right)
            if successor.parent != node: # преемник не прямой наследник
                self._transplant(successor, successor.right) # готовим преемника к переезду
                successor.right = node.right # ставим преемника на место уд.узла
                if successor.right:
                    successor.right.parent = successor # обновляем ссылку на потомка
